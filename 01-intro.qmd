...
We can measure the length of a spatial line feature.

Spatial Polygon Data
: Geometries in this type of data are polygons.
Actually, polygons are stored as a seires of points, which looks like closed polygonal chain.
But the difference lies in that a polygon consists of both the polygonal chain (boundary) and area in it.
Thus, for polygons we can measrue their area, but for spatial line data we cannot.
...
To storage spatial data properly on our computer, there are tens and hundreds of file formats desgined in the past decades.
In this section, we will only introduce some popular formats, which can be accepted by most geographical infromation softwares.

#### Comma Seperated Values (CSV)

This format is probibly the most popular one.
In a CSV file which is actually a table, each row represents a entity/record.
Columns are seperated by commas, representing attributes.
Geometry information is stored in a special column.
For spatial point data which we only need coordinates of those points, there are some columns telling us values of every dimension,
...
When we talk about the formats of spatial data, we need to emphesis its difference from file extension name on computer.
